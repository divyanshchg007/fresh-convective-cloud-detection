# original: Cornelia Klein
# modified by Divyansh Chug
...
def era_Tlapse_height(month, year, temp, lon, lat):
    """
    ...
    """
    path  = '/data/keeling/a/dchug2/a/ERA5/monthly/nwindia_z_t/'
    ...
    g = 9.80665
    t = da['t'] # for comparison with cloud top T in K
    z = da['z']
    zm = z/g  ## geopotential / gravity constant
    ismin = np.argmin(t.values)
    #print(t.values[ismin])
...
def call_parallax_era(month, year, t_cloud, lon_cloud, lat_cloud, lon_sat, lat_sat):

    height = era_Tlapse_height(month, year, t_cloud, lon_cloud, lat_cloud)  # height in meters
    sat_height = 36000 # MSG satellite height in km
    km_coords = parallax_correction(lon_sat, lat_sat, lon_cloud, lat_cloud, height/1000, sat_height)
